...
@[target] theorem card_transGen_lt (hr : ∀ y, #{x | r x y} < κ) (hκ₁ : κ.IsRegular) (hκ₂ : ℵ₀ < κ) (y : α) :
    #{x | TransGen r x y} < κ := by sorry


@[target] theorem card_reflTransGen_le (hr : ∀ y, #{x | r x y} ≤ κ) (hκ : ℵ₀ ≤ κ) (y : α) :
    #{x | ReflTransGen r x y} ≤ κ := by
  sorry


@[target] theorem card_transGen_le (hr : ∀ y, #{x | r x y} ≤ κ) (hκ : ℵ₀ ≤ κ) (y : α) :
    #{x | TransGen r x y} ≤ κ := by sorry
