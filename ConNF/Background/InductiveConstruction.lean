...
@[target] theorem core_dom (hp : ∀ i d h, p i (f i d h) d) : ∀ i, (core f i).Dom := by
  sorry


noncomputable def fix : ∀ i, α i := by sorry
...
@[target] theorem fix_eq (i : I) :
    fix f hp i = f i (λ j _ ↦ fix f hp j) (λ j _ ↦ fix_prop f hp j) := by sorry
